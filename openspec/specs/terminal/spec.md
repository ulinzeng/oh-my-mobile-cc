...
## Purpose
提供 CC 子进程 PTY 输出的 **只读** 终端视图（W1 阶段）。
本 capability 覆盖 VT100/ANSI 转义序列解析、East Asian Width + Emoji 宽度计算、
基于 Compose Multiplatform canvas 的纯自绘网格渲染。
若 W3 第 7 天 checkpoint 自绘终端稳定性不达标，降级到 xterm.js 嵌入 WebView。
## Requirements

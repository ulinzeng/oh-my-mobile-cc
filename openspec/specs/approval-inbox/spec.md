...
用户在 iOS/Android app 中收到 CC 通过 `PreToolUse` hook 转来的工具调用请求，
可 Allow / Deny / Customize，决定经由本地 IPC 回到 hook 子进程的 stdout
（以 `hookSpecificOutput.permissionDecision` 形式），最终放行或拒绝 CC 的工具调用。
本 capability 明确 **不** 依赖 APNs：10 分钟无响应自动 DENY + `expired`。
详见 `docs/adr/0004-approval-bridge-via-pretooluse-hook.md`。
## Requirements
